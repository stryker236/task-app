    ...
    input: [
      {
        ...
        content: [
          ...
          'Prefer reusing availableTags exactly as written when an existing tag clearly fits the task.',
          'availableTags is not a closed vocabulary. Suggest new reusable tags when they describe the task better than the existing pool.',
          'It is acceptable to add 1-3 new tags for a task when the title/notes reveal a useful domain, project, context, energy, location, or scheduling category.',
    ...
    input: [
      {
        ...
        content: [
          ...
          'Prefer reusing availableTags exactly as written when an existing tag clearly fits the task.',
          'availableTags is not a closed vocabulary. Create new reusable tags when they describe the task better than the existing pool.',
          'For untagged tasks with enough signal, it is acceptable to suggest 1-3 new tags even if there are no close matches in availableTags.',
      ...
      },
      {
        ...
        content: JSON.stringify({
          ...
          availableTags: tags.map((tag) => tag.name || tag).slice(0, 200),
          newTagPolicy: {
            availableTagsAreClosedVocabulary: false,
            allowNewTags: true,
            preferExistingTagsWhenClearlyRelevant: true,
            allowNewTagsForDomainProjectContextEnergyLocationOrScheduling: true,
            maxNewTagsPerTaskGuideline: 3
          },
          advisorMemory: memory,
          untaggedTaskIds,
