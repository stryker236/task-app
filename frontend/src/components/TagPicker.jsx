    ...
    <div className="dependency-field">
      ...
      <div className="selected-dependencies">
        ...
        {selected.map((tag) => (
          <span className="dependency-chip tag-chip" key={normalize(tag)}>
            #{tag}
            <button type="button" aria-label={`Remover tag ${tag}`} onClick={() => toggle(tag)}>×</button>
          </span>
        ))}
      </div>
      ...
      {open && (
        <div className="modal-backdrop" role="presentation" onMouseDown={() => setOpen(false)}>
          <div className="dialog dependency-dialog" role="dialog" aria-modal="true" aria-labelledby="tag-picker-title" onMouseDown={(event) => event.stopPropagation()}>
