...
export default function TaskCard({ task, allTasks, onEdit, onDelete, onDuplicate, onStatusChange, onOpenTask, dragEnabled = false, onDragStart, onDragEnd }) {
  ...
  return (
    <article className={`task-card priority-${task.priority} ${unfinished.length ? 'is-blocked' : ''}`}>
      ...
      <div className="task-meta">
        ...
        <span>{task.dueDateTime ? new Intl.DateTimeFormat('pt-PT', { dateStyle: 'short', timeStyle: 'short' }).format(new Date(task.dueDateTime)) : 'Sem prazo'}</span>
      </div>
